...
function dirReduc(arr){
  ...
  return arr.reduce(function reduce(total, value, i, arr){

    for( i=0; i<total.length;i++){
      if( total[i] == 'NORTH' && total[i+1] == 'SOUTH' ||
          total[i] == 'SOUTH' && total[i+1] == 'NORTH' ||
          total[i] == 'WEST' && total[i+1] == 'EAST' ||
          total[i] == 'EAST' && total[i+1] == 'WEST'
      ){
        total.splice(i,2);
        reduce(total,0);
      }
