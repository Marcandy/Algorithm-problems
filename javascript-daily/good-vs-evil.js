...
function goodVsEvil(good, evil){
  //loop through ech each arr and mulitply by corresponding
  let arrg = [1, 2, 3, 3, 4, 10],
      arrv = [1, 2, 2, 2, 3, 5, 10];

  let  good2 = good.split(' ').map(function(elm, i) {
    return elm * arrg[i];
  })

  let evil2 = evil.split(' ').map(function(elm, i) {
    return elm * arrv[i];
  })

  let totalg = good2.reduce(function(a, b) {
    return a + b
  },0)

  let totalv = evil2.reduce(function(a, b) {
    return a + b
  },0)
