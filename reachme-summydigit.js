...
function sumDigNthTerm(initval, patternl, nthterm) {
    // your code
    let total = initval;
    let loop = patternl.length -1; // first map through the last element keep incrementing by initial value
    for(var i = 1, j= 0; i < nthterm ; i++) {
      total += patternl[j]
    ...
    return String(total).split('').reduce(function (a, b) {
      return Number(a) + Number(b)
    }, 0)

}

// other way a bit broken
function sumDigNthTerm(initval, patternl, nthterm) {
    // your code
    var j = 0;
    ...
    return sum = (initval + '').split('').reduce(function (a, b) {
      return Number(a) + Number(b)
    }, 0)
